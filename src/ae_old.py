...
class AEDense:
    ...
    def __init__(
        ...
        n_layers_encoder: "list",
        n_latent_dimensions: "int",
        n_layers_decoder: "list",
        batch_size: "int",
        epochs: "int",
        learning_rate: "float",
        loss: "str",
        ...
        self.inputs = Input(
            shape=(self.n_input_dimensions,), name="ae_input_layer"
        )
    ...
    def build_ae(self):
        ...
        ae.compile(
            loss=self.loss, optimizer=adam_optimizer, metrics=["accuracy"]
        )
    ...
    def build_encoder(self) -> "tf.keras.model":
        ...
        X = self.inputs
        std_dev = np.sqrt(2.0 / self.n_input_dimensions)

        for idx, n_units in enumerate(self.n_layers_encoder):

            w_init = keras.initializers.RandomNormal(mean=0.0, stddev=std_dev)

            layer = Dense(
                n_units,
                name=f"encoder_layer_{idx+1}",
                activation="relu",
    ...
    ###########################################################################
    def _latent_layer(
        self, n_units: "int", X: "tf.keras.Dense"
    ) -> "tf.keras.Dense":
    ...
    def build_decoder(self) -> "tf.keras.model":
        ...
        input_decoder = Input(
            shape=(self.n_latent_dimensions,), name="decoder_input"
        )

        std_dev = np.sqrt(2.0 / self.n_latent_dimensions)
...
class VAEDense:
    ...
    def _stochastic_layer(
        ...
        self.latent_mu = Dense(
            self.n_latent_dimensions,
            name="latent_mu",
            kernel_initializer=w_init,
        )(X)

        self.latent_ln_sigma = Dense(
            self.n_latent_dimensions, name="latent_ln_sigma"
        )(
            X
        ...
        ########################################################################
        latent = Lambda(
            _sample_latent_features,
            output_shape=(self.n_latent_dimensions,),
            name="latent",
        )([self.latent_mu, self.latent_ln_sigma])
...
class DenseEncoder:
    ...
    def __init__(
        ...
        self.inputs = Input(
            shape=(self.n_input_dimensions,), name="encoder_input_layer"
        )
    ...
    def stochastic_layer(self, n_units, X):
        ...
        self.latent_ln_sigma = Dense(
            self.n_latent_dimensions,
            name="latent_ln_sigma",
            kernel_initializer=w_init,
        )(X)

        latent = Lambda(
            self._sample_latent_features,
            output_shape=(self.n_latent_dimensions,),
            name="latent",
        )([self.latent_mu, self.latent_ln_sigma])
