...
if os.path.exists(reconstructed_test_set_path):
    ...
    reconstructed_test_set = load_data(
        reconstructed_test_set_name, reconstructed_test_set_path
    )
else:
    ...
    model_tail = (
        f"{layers_str}_loss_{loss}_nTrain_{number_spectra}_"
        f"nType_{normalization_type}"
    )
    #    f'nType_{normalization_type}')
    if local:
...
for metric in metrics:
    ...
    ################################################################################
    scores_test = outlier.score(
        O=test_set[:, :-8], R=reconstructed_test_set, percentages=percentages
    )
    ...
    for idx, scores in enumerate(scores_test):
        ...
        ############################################################################
        scores_name = (
            f"{test_set_name}_{metric}_score_{percent_str}_{tail_model_name}"
        )
        ...
        spec_top_outliers = np.insert(
            spec_top_outliers, 0, outlier_ids, axis=1
        )

        np.save(
            f"{scores_dir}/{spec_top_outliers_name}.npy", spec_top_outliers
        )
        ########################################################################
        R_top_outliers_name = (
